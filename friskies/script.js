...
function createImageCollection(rowAmount, colAmount) {
  ...
  while (pickedIndices.length < imageAmount) {
    var index = Math.floor(Math.random() * (imageAmount));
  ...
  for (var m = 0; m < rowAmount; m++) {
    for (var n = 0; n < colAmount; n++) {
      ...
      if (cardStyleSheet.cssRules[styleCount]) {
        ...
        cardStyleSheet.cssRules[styleCount].style.backgroundImage = `url(./images/cards/${newCard.image})`;
      } else {
        if (cardStyleSheet.addRule) {
          cardStyleSheet.addRule(`#${cardId}-b`,
            `
              background-image: url(./images/cards/${newCard.image});
            `,
            styleCount);
        } else {
          frontSideCard.style.backgroundImage = `url(./images/cards/${newCard.image})`;
        }
      }
      styleCount ++;
